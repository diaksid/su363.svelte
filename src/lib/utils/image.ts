...
export async function dominantColour({ source }: never) {
  try {
...
export async function lowResolutionPlaceholder({ source }: never) {
  try {
    const image = sharp(source);
...
export async function metadata({ source }: never) {
  try {
    const image = sharp(source);
